...
public class MeshCombineUtility {
	...
	public static Mesh Combine (MeshInstance[] combines, bool generateStrips)
	{
		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh)
			{
				...
				if (generateStrips)
				{
                    ...
                    int curStripCount = combine.mesh.GetTriangles(combine.subMeshIndex).Length;//  GetTriangleStrip(combine.subMeshIndex).Length;
                    if (curStripCount != 0)
					{
						if( stripCount != 0 )
						{
        ...
        if (generateStrips)
            ...
            mesh.SetTriangles(strip, 0);//   SetTriangleStrip(strip, 0);
        else
